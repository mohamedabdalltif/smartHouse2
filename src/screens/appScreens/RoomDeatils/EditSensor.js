...
const EditSensor = ({ navigation, route }) => {
    ...
    const [slider_min_val, setSlider_min_val] = React.useState(psdata.min_value);
    const [slider_max_val, setSlider_max_val] = React.useState(psdata.max_value);
    ...
    const editSensor = () => {
        ...
        // console.log(psdata)
            ...
            const data_send = {
                sensor_id:psdata.sensor_id,
                name: name,
                 topic:topic
            }
            // console.log(data_send)
            
            axios.post("https://camp-coding.tech/smart_home/sensors/update_sensors.php", data_send).then((res) => {
                console.log(res.data)
                if (res.data.status == "success") {
                    utils.toastAlert("success", "Sensor edited successfully")
    ...
    const deleteSensor=()=>{
        setDelLoading(true)
        axios.post("https://camp-coding.tech/smart_home/sensors/delete_sensors.php",{
            sensor_id:psdata.sensor_id
        }).then((res)=>{
            if (res.data.status == "success") {
                utils.toastAlert("success", "Sensor deleted successfully")
                navigation.navigate("Home")
                setDelLoading(false)
            } else {
                utils.toastAlert("error", "Error happen please try again later")
